...
// Defining Tama's livelihood
  boolean tamaIsEgg = true;
  boolean tamaIsAlive = false;  
  int tamaDeathTreshold = 99800;
 
// Hunger related
  int tamaHungerFull = 99999;                              // Tama's maximum hunger value
  int tamaHunger = tamaHungerFull;                         // Hunger value
  int tamaOriginalHungerSpeed = 1;
  int tamaHungerSpeed = tamaOriginalHungerSpeed;           // How fast does tama get hungry?
  
// Thirst Related
  int tamaThirstFull = 99999;                              // Tama's maximum Thirst value
  int tamaThirst = tamaThirstFull;                         // Thirst value
  int tamaOriginalThirstSpeed = 1;
  int tamaThirstSpeed = tamaOriginalThirstSpeed;           // How fast does tama get thirsty?
  
  // Sleepiness related
  int tamaTiredFull =  99999;                              // Tama's maximum sleepiness value
  int tamaTired = tamaTiredFull;                           // Energy value
  int tamaOriginalTiredSpeed = 1;
  int tamaTiredSpeed = tamaOriginalTiredSpeed;             // How fast does tama get tired?
 
// Illness related
  boolean tamaIsIll = false;                               // Init tama's ilness state
  float  tamaIllnessChance = 0.008;                        // How big is the chance tama gets ill?
  
// Tantrum related
  boolean tamaTantrum = false;                             // Init tama's tantrum state
  float  tamaTantrumChance = 0.008;                        // How big is the chance tama throws a tantrum?
